...
export class Value {
  ...
  get errorList() {
    let validateErrorList = filterErrorListByKeyPath(this.root._errorList, this.keyPath);
    let externalErrorList = filterErrorListByKeyPath(
      this.root._externalErrorList,
      this.keyPath,
    );
  ...
  update(valueUpdate, suppressUpdate) {
    log("Value.update keyPath=%o value=%o", this.keyPath, valueUpdate);
    let value;
