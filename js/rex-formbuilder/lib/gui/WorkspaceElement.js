...
function canDropInside(props, monitor) {
  ...
  if (!element.constructor.isContainingElement()) return false;

  //Item coudn't be a subfield
  if (!item.element.constructor.canBeSubField()) return false;

  //if element is already a subfield we can't add 3 level
  if (props.isSubField) return false;
...
const workspaceElementTarget = {
  ...
  hover(props, monitor, component) {
    ...
    //coudn't be subfield
    if (props.isSubField && !item.element.constructor.canBeSubField()) return;
    DraftSetActions.putElement(item.element, element);
  }
};

export default DropTarget(
  [WORKSPACE_ELEMENT, ELEMENT_TYPE],
  workspaceElementTarget,
  connect => ({
  ...
  DragSource(WORKSPACE_ELEMENT, workspaceElementSource, (connect, monitor) => ({
    ...
    class WorkspaceElement extends React.Component {
      ...
      constructor(props) {
        super(props);
        let needsEdit =
          this.props.element.needsEdit || this.props.element.forceEdit || false;
      ...
      canMove() {
        return !this.props.fixed && !this.state.editing && !this.state.deleting;
      }
            ...
            <div className={classes}>
              ...
              <div className="rfb-workspace-item-tools">
                ...
                <button
                  ...
                  title={
                    this.props.fixed
                      ? _("You Cannot Delete this Element")
