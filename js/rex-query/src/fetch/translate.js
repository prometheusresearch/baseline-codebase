...
function translateExpression(
  ...
  switch (query.name) {
    ...
    case "unary":
      return [
        UNARY_OP_ENCODE[query.op],
        translateExpression(query.expression, prev)
      ];
  ...
  switch (query.name) {
    ...
    case "define": {
      ...
      const selectTranslation = translateQuery(
        select != null
          ? q.pipeline(q.navigate(path), select)
          : q.navigate(path),
        { translated: HERE, context: query.context },
        options
      );
      const bindingWithSelect = [
        "=>",
        query.binding.name,
        selectTranslation.translated
      ];
      const translated = [
        "define",
        prev.translated,
    ...
    case "select": {
      ...
      for (const k in query.select) {
        if (query.select.hasOwnProperty(k)) {
          ...
          if (
            options.limitSelect != null &&
            field.context.type.card === "seq"
          ) {
