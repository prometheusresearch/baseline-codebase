...
describe("inferType()", function() {
  ...
  it("infers type of query atoms", function() {
    ...
    expect(
      inferQueryType(voidContext(domain), individual).context.type
    ).toEqual(seqType(entityType(domain, "individual")));
    expect(
      inferQueryType(voidContext(domain), navigate("unknown")).context.type
    ).toEqual(invalidType(domain));
    expect(
      inferQueryType(voidContext(domain), filter(value(true))).context.type
    ).toEqual(voidType(domain));
    expect(inferQueryType(voidContext(domain), limit(10)).context.type).toEqual(
      voidType(domain)
    );
    expect(
      inferQueryType(voidContext(domain), select({ a: pipeline(individual) }))
        .context.type
    ).toEqual(
      recordType(domain, {
        a: {
          title: "Individual",
          type: seqType(entityType(domain, "individual"))
        }
      })
    );
    expect(
    ...
    expect(
      inferQueryType(voidContext(domain), aggregate("count")).context.type
    ).toEqual(invalidType(domain));
  });

  it("fails on navigate to unknown attribute", function() {
    expect(
      inferQueryType(voidContext(domain), pipeline(individual, navigate("b")))
        .context.type
    ).toEqual(invalidType(domain));
  });
  ...
  it("individual.name", function() {
    ...
    expect(
      stripDomain(inferQueryType(voidContext(domain), query))
    ).toMatchSnapshot();
  });
  ...
  it("individual:define(s := sample:count()):select(s:select(title))", function() {
    ...
    expect(
      stripId(inferQueryType(voidContext(domain), query))
    ).toMatchSnapshot();
  });
...
describe("inferExpressionType(context, expression)", function() {
  ...
  it("customer < 42", function() {
    expect(
      stripId(
        inferExpressionType(
          voidContext(domain),
          less(navigate("customer"), value(42))
        )
      )
    ).toMatchSnapshot();
  });
  ...
  it("customer and 42", function() {
    expect(
      stripId(
        inferExpressionType(
          voidContext(domain),
          and(navigate("customer"), value(42))
        )
      )
    ).toMatchSnapshot();
  });
  ...
  it("not customer", function() {
    expect(
      stripId(
        inferExpressionType(voidContext(domain), not(navigate("customer")))
      )
    ).toMatchSnapshot();
  });
  it("exists customer", function() {
    expect(
      stripId(
        inferExpressionType(voidContext(domain), exists(navigate("customer")))
      )
    ).toMatchSnapshot();
  });
});
...
test("genQueryName (untyped)", function() {
  ...
  expect(genName(pipeline(navigate("customer"), navigate("name")))).toBe(
    "Customer Name"
  );
  ...
  expect(
    genName(pipeline(navigate("customer"), aggregate("count", "name")))
  ).toBe("Customer Name count");
});

test("genQueryName (typed)", function() {
  let genName = query =>
    genQueryName(inferQueryType(voidContext(domain), query));
  expect(genName(pipeline(navigate("customer")))).toBe("Customer");
  expect(genName(pipeline(navigate("customer"), navigate("name")))).toBe(
    "Customer Name"
  );
...
test("genExpressionName", function() {
  ...
  expect(genExpressionName(or(lessEqual(navigate("name"), value(42))))).toBe(
    "name"
  );
});
