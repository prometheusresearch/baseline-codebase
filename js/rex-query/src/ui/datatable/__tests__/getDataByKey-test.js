...
test("plain list / accessing unknown key", function() {
  ...
  expect(getDataByKey(transposedData[0], ["record", "xxx"], focus)).toEqual(
    undefined
  );
});
...
test("plain list with nested record", function() {
  ...
  const transposedData = transpose(data, focus);
  expect(getDataByKey(transposedData[0], ["record"], focus)).toEqual(
    data.record[0]
  );
  ...
  expect(getDataByKey(transposedData[0], ["record", "b", "c"], focus)).toEqual(
    11
  );
  expect(getDataByKey(transposedData[1], ["record"], focus)).toEqual(
    data.record[1]
  );
  ...
  expect(getDataByKey(transposedData[1], ["record", "b", "c"], focus)).toEqual(
    21
  );
});
...
describe("nested list", function() {
  ...
  describe("focus: record", function() {
    ...
    test("row 0", function() {
      ...
      const values = keyPathSet.map(keyPath =>
        getDataByKey(row, keyPath, focus)
      );
