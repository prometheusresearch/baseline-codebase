  ...
  describe('ContextUtils', function() {
    describe('getMaskedContext', function() {
      it('returns a piece of context described by type', function() {
        ...
        assert.deepEqual(getMaskedContext({a: 1, b: 2}, inputType), {a: 1, b: 2});
        assert.deepEqual(getMaskedContext({a: 1, b: 2, c: 3}, inputType), {a: 1, b: 2});
        assert.deepEqual(getMaskedContext({a: 1, b: null}, inputType), {a: 1});
      });
    });
    ...
    describe('contextToParams', function() {
      it('applies context to producible', function() {
        ...
        assert.deepEqual(contextToParams({entity: createEntity('obj', 1)}, inputType), {
          ':value': undefined,
          ':entity': 1,
        });
