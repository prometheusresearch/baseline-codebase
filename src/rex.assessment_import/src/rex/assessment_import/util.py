...
def make_template(fields, obj_output_name, required=None, output={}):
    ...
    for field_id, field in fields.items():
        ...
        if field.base_type == 'enumeration':
            ...
            template[field_id] = (tpl_string % tpl_data).strip()
            ...
            rec_output_name = obj_output_name + '.' + field_id
            output[rec_output_name] = OrderedDict()
            output = make_template(field.fields, rec_output_name, output=output)
        elif field.base_type == 'matrix':
            for _, row in field.rows.items():
                required = row.required
                for _, column in row.columns.items():
                    column_field_id = row.id + '_' + column.id
                    output = make_template({column_field_id: column.field},
                                            obj_output_name,
                                            required, output)
        else:
            template[field_id] = (tpl_string % tpl_data).strip()
