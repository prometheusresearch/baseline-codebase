...
class IdentityModel(Model):
    ...
    @classmethod
    def _make_uuid_key(cls, table, column):
        # Builds code for autogenerated UUID primary key.
        is_link = len(column.foreign_keys) > 0
        type_qname = (column.type.schema.name, column.type.name)
        if type_qname == ('pg_catalog', 'text') and not is_link:
            return plpgsql_text_uuid_key(table.qname, column.name)
        else:
            raise Error("Expected a text column:", column)
    ...
    def _generate(cls, table, generators):
        ...
        for column, generator in zip(table.primary_key, generators):
            if generator == 'offset':
                ...
                source.append(cls._make_uuid_key(table, column))
        if not source:
            return None
