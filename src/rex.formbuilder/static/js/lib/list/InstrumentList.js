...
var InstrumentVersions = React.createClass({
  ...
  makeURL(uid, group) {
    return format(this.props.editorURLTemplate, {
  ...
  render() {
    ...
    var versions = items.map((item) => (
      <div key={item.uid} className="row rfb-instrument-version">
        ...
        <div className="col-md-3">{item.desc}</div>
        <div className="col-md-2">{item.time}</div>
        <div className="col-md-3">
          <a className="btn btn-default"
              href={this.makeURL(item.uid, groupName)}>
...
var Instrument = React.createClass({
  ...
  render() {
    var {instrument, expanded, activeTab, updatingStatus,
      editorURLTemplate} = this.props;
    var className = cx({
      'panel-collapse': true,
      'collapse': true,
      'in': expanded
            ...
            <ul className="rfb-tab-content">
              <li className={publishedClassName}>
                {instrument.published.items?
                  <InstrumentVersions
                    editorURLTemplate={editorURLTemplate}
...
var InstrumentList = React.createClass({
        ...
        <div className="rfb-InstrumentList__list">
          ...
          <div className="panel-group" id="accordion">
            {this.state.instruments.map((instrument) => (
              <Instrument 
                editorURLTemplate={this.props.editorURLTemplate}
  ...
  onCreateVersion(instrument) {
    InstrumentListStore.createInstrumentVersion(instrument);
  },

  onCreateInstrument(code, title) {
    InstrumentListStore.createInstrument(code, title);
  }
});
