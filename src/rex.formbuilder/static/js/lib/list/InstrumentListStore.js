...
var InstrumentListStore = merge({
    ...
    var group = merge(instrument[groupName], {
      items: response.body.map((item) => {
        ...
        if (item.published_by) {
          ...
          time = this._friendlyDate(item.date_published);
        } else if (item.modified_by) {
          desc = 'Modified by: ' + item.modified_by;
          time = this._friendlyDate(item.date_modified);
        } else if (item.created_by) {
          desc = 'Created by: ' + item.created_by;
          time = this._friendlyDate(item.date_created);
        }
        return {
          'uid': item.uid,
          'desc': desc,
   ...
   */
  createInstrumentVersion(instrument) {
    var definition = {
    ...
    API.createInstrumentVersion(instrument.uid, definition).then(
   ...
   */
  createInstrument(code, title) {
    API.createInstrument(code, title, 'disabled').then(
      this._onInstrumentCreated.bind(this),
      this._onInstrumentVersionCreatedError.bind(this)
    );
  },

  _onInstrumentCreated(response) {
    var {uid, title} = response.body;
    this.createInstrumentVersion({uid, title});
  },
