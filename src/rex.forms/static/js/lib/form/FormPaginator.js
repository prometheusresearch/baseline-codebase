...
import * as Focus from '@prometheusresearch/react-ui/lib/Focus';
import {NextIcon, PrevIcon} from '../icons';

import {InjectI18N} from 'rex-i18n';

@InjectI18N
export default class FormPaginator extends React.Component {
  render() {
    let {
      currentPageNumber,
      pageCount,
    ...
    if (prevButtons.length + nextButtons.length > 10) {
      if (prevButtons.length > 5) {
        prevButtons = prevButtons
          .slice(0, 2)
          .concat(
            <EllipsisButton key="__prevEllipsis__" />
          )
      ...
      if (nextButtons.length > 5) {
        nextButtons = nextButtons
          .slice(0, 2)
          .concat(
            <EllipsisButton key="__nextEllipsis__" />
          )
...
function PageButton({onPage, pageNumber, active, hidden, style, children, ...props}) {
