...
export default class Question extends React.Component {
  ...
  render() {
    let {
      formValue, question, plain, editable, disabled, mode
    } = this.props;
    ...
    let renderExplanation = (
      formValue.schema.properties.explanation
    );
    ...
    return (
      <Component
        ...
        <QuestionValue
          ...
          />
        {renderExplanation &&
          <ReactUI.Block marginStart="small" marginBottom={renderAnnotation ? 'small' : undefined}>
            <Explanation
              disabled={disabled}
              readOnly={readOnly}
          ...
          <ReactUI.Block position="absolute" top={10} positionEnd={10}>
            <ReactUI.Block inline marginRight="xx-small">
              <ReactUI.FlatSuccessButton
                disabled={formValue.select('value').completeErrorList.length > 0}
                size="small"
            ...
            <ReactUI.Block inline>
              <ReactUI.FlatDangerButton size="small" onClick={this.onCancelEdit}>
                {this._('Cancel')}
  ...
  onCommitEdit = () => {
    if (this.props.formValue.select('value').completeErrorList.length ===  0) {
