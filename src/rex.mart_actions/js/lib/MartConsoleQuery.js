  ...
  render() {
    ...
    return (
      <Element height="100%">
        ...
        <Element position="relative" height={`calc(100% - ${CONSOLE_TOOLBAR_HEIGHT}px)`}>
          <ui.IFrame style={{padding: 1}} ref={this.onIFrameDidMount} src={htsqlUrl} />
        </Element>
      </Element>
    );
  }
...
@Fetch(function fetchQuery({fetchQuery, context}) {
  const query = fetchQuery.params({'*': context.mart_console_query.id}).getSingleEntity();
  return {query};
})
export class MartEditConsoleQuery extends React.Component {
  static defaultProps = {
    icon: 'eye-open',
  };
  ...
  onSave = async () => {
    ...
    this.api
      .update({id, title, query})
      .then(this.onSaved.bind(null, 'Query saved successfully'), this.onError);
  };
  ...
  onClone = async () => {
    ...
    this.api
      .insert({mart, title, query})
      .then(this.onSaved.bind(null, 'Query cloned successfully'), this.onError);
  };

  onSaved = (message, data) => {
    this.removeProgress();
    ui.showNotification(<ui.Notification kind="success" text={message} icon="ok" />);
    let newEntity = Object.values(data)[0][0];
  ...
  render() {
    const {context, cloneQuery, fetched: {query}, ...props} = this.props;
    const {saving, title} = this.state;
