...
class LocalDriver(Driver):
    ...
    def __init__(self, key: str, secret: str = None, salt: str = None,
        ...
        self.base_path = os.path.normpath(os.path.abspath(key))
        self.salt = salt
    ...
    def _make_blob(self, container: Container, object_name: str) -> Blob:
        ...
        # we have to use `os.path.normapth` here since Path.resolve() also
        # resolves the symlinks - which we don't want here
        container_path = pathlib.Path(os.path.normpath(base_path / container.name))
        if base_path not in container_path.parents:
            raise NotFoundError(messages.CONTAINER_NOT_FOUND % container.name)

        object_path = pathlib.Path(os.path.normpath(container_path / object_name))
        if container_path not in object_path.parents:
            raise NotFoundError(messages.BLOB_NOT_FOUND % (object_name,
                                                           container.name))
        full_path = str(object_path)
    ...
    def upload_blob(self, container: Container, filename: FileLike,
        ...
        container_path = pathlib.Path(
                self._get_folder_path(container, validate=True)
            ).resolve(strict=False)

        blob_name = blob_name or validate_file_or_path(filename)
        blob_path = (container_path / blob_name).resolve(strict=False)
        if container_path not in blob_path.parents:
