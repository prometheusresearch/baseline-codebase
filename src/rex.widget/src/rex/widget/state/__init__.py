"""

    rex.widget.state
    ================

    :copyright: 2014, Prometheus Research, LLC

"""

from collections import namedtuple
from rex.core import Validate, Error
from .computator import (
        CollectionComputator,
        PaginatedCollectionComputator, IdentityComputator)
from .graph import parse_ref


StateDescriptor = namedtuple(
        'StateDescriptor',
        ['id', 'value', 'dependencies', 'rw'])


class StateGenerator(object):
    """ Abstract base class for fields which can generate state."""

    def describe_state(self, widget_id, field_name):
        """ This method should return a list of pairs
        ``(prop_name, StateDescriptor)``."""
        raise NotImplementedError(
            "%s.describe_state(name) is not implemented" % self.__class__.__name__)


class DataReference(StateGenerator):

    def __init__(self, computator_factory, url, refs=None, include_meta=False):
        self.computator_factory = computator_factory
        self.url = url
        self.refs = refs
        self.include_meta = include_meta

    def create_computator(self, extra_refs=None):
        refs = self.refs

        if extra_refs is not None:
            refs = dict(self.refs)
            refs.update(extra_refs)

        return self.computator_factory(self.url, refs, self.include_meta)

    def describe_state(self, widget_id, field_name):
        state_id = "%s.%s" % (widget_id, field_name)
        return [
            (field_name,
                StateDescriptor(
                    state_id,
                    self.create_computator(),
                    dependencies=[r.id for r in self.refs.values()],
                    rw=False))
        ]


class PaginatedCollectionReference(DataReference):
    """ A reference to a collection which provides pagination mechanism."""

    def describe_state(self, widget_id, field_name):
        state_id = "%s.%s" % (widget_id, field_name)
        pagination_state_id = "%s.%s.pagination" % (widget_id, field_name)

        dependencies = [r.id for r in self.refs.values()]

        refs = dict(self.refs)
        refs.update({
            "top": "%s:top" % pagination_state_id,
            "skip": "%s:skip" % pagination_state_id,
        }),

        return [
            (field_name,
                StateDescriptor(
                    state_id,
                    PaginatedCollectionComputator(
                        pagination_state_id,
                        self.url,
                        refs=refs,
                        include_meta=self.include_meta),
                    dependencies=dependencies + [pagination_state_id],
                    rw=False)),
            ("%sPagination" % field_name,
                StateDescriptor(
                    pagination_state_id,
                    IdentityComputator(
                        {"top": 100, "skip": 0},
                        dependencies=dependencies),
                    dependencies=dependencies,
                    rw=True)),
        ]


class State(StateGenerator):

    def __init__(self, initial):
        self.initial = initial

    def describe_state(self, widget_id, field_name):
        state_id = "%s.%s" % (widget_id, field_name)
        return [
            (field_name,
                StateDescriptor(
                    state_id, self.initial,
                    dependencies=[], rw=True))
        ]


class DataReferenceVal(Validate):

    data_reference_factory = DataReference

    def __init__(self, include_meta=False):
        self.include_meta = include_meta

    def __call__(self, data):
        if isinstance(data, basestring):
            return self.data_reference_factory(
                    self.computator_factory,
                    data,
                    refs={},
                    include_meta=self.include_meta)
        elif isinstance(data, dict):
            if not "url" in data:
                raise Error(
                    "invalid data reference: expected an URL or "
                    "{url: ..., refs: ...} mapping")
            refs = {name: parse_ref(ref)
                    for name, ref
                    in data.get("refs", {}).items()}
            return self.data_reference_factory(
                    self.computator_factory,
                    data["url"],
                    refs=refs,
                    include_meta=self.include_meta)
        else:
            raise Error(
                "invalid data reference: expected an URL or "
                "{url: ..., refs: ...} mapping")


class CollectionReferenceVal(DataReferenceVal):

    computator_factory = CollectionComputator


class PaginatedCollectionReferenceVal(DataReferenceVal):

    data_reference_factory = PaginatedCollectionReference
    computator_factory = PaginatedCollectionComputator


class StateVal(Validate):

    def __init__(self, validate):
        if isinstance(validate, type):
            validate = validate()
        self.validate = validate

    def __call__(self, data):
        return State(self.validate(data))
